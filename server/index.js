    ...
    client.query("SELECT * FROM users WHERE email = $1 ORDER BY id ASC",
      ...
      function(err, result) {
        ...
        if (user) {
          ...
          client.query('INSERT INTO users(email, password, firstname, lastname, verify_token, role) values($1, $2, $3, $4, $5, $6) RETURNING *',
            [data.email, data.password, data.firstName, data.lastName, data.verify_token, data.role],
            function(err, result) {
              if (err) {
                return res.status(500).json({ success: false, data: err });
              } else {
